...
3. Add the backend selector and port to the appropriate Cilium gateway policy in `helm/network-policies/templates/`.
4. For internal services, add a Pi-hole DNS entry pointing to `192.168.1.50`. For public services, use `traefik-public` and a Cloudflare Tunnel hostname route.
5. Deploy the Helm release and verify the endpoint and denied flows in Hubble.

Never point Cloudflare Tunnel at the internal Traefik controller. Public applications must use a `public` namespace, ClusterIP Services, and narrowly scoped reciprocal network policies.
